...
TARGET="/mnt/shinnosuke"

[ "$(pwd)" == "$SRC" ] || cd "$SRC"

[ -n "$BRANCH" ]    || BRANCH="master"
[ -n "$NR_PROC" ]   || NR_PROC="$(nrproc)"
[ -n "$SRC_FILES" ] || SRC_FILES="$(pwd)/files-asus-rog-strix-z690-i-gaming-wifi"
...
# Launch the build
_NR_PROC=$(( ${NR_PROC:-1} > 24 ? 24 : ${NR_PROC:-1} ))
[ ! -v SKIP_UPDATE ] && ( taskset -c $(( 24 - $_NR_PROC ))-23 make download -j"$_NR_PROC" V=sc || make download V=sc )
taskset -c $(( 24 - $_NR_PROC ))-23 make world -j"$_NR_PROC" V=sc

# List built firmwares
ls -Altr bin/targets/x86/64
ls -Ahltr bin/targets/x86/64
