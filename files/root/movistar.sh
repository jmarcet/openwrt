...
openwrt_revision() {
	echo $DISTRIB_REVISION | sed -e 's/^r//'
}
...
router_detect() {
	...
	if [ "$(openwrt_revision)" -ge "47867" ] && [ -f $board_json ]; then
		json_init
		json_load "$(cat ${board_json})"

		# Detect device model
		json_select model
			local id name
			json_get_vars id name
			router=$id
			router_name=$name
		json_select ..
	else
...
EOF
	...
	case $router in
		...
		"armada-xp-linksys-mamba" |\
		"wap-5813n")
			ucicfg_add_switch "switch0" "1" "1" >> $debug 2>&1
			ucicfg_add_switch_vlan "switch0" "1" "0 1 2 3 5t" >> $debug 2>&1
			if [ $iptv_enabled -eq 1 ]; then
				ucicfg_add_switch_vlan "switch0" "2" "4t 5t" >> $debug 2>&1
			fi
			if [ $voip_enabled -eq 1 ]; then
				ucicfg_add_switch_vlan "switch0" "3" "4t 5t" >> $debug 2>&1
			fi
			ucicfg_add_switch_vlan "switch0" "6" "4t 5t" >> $debug 2>&1
			;;
		"archer-c5" |\
		"archer-c7" |\
		"tl-wdr4900-v2")
			ucicfg_add_switch "switch0" "1" "1" >> $debug 2>&1
			ucicfg_add_switch_vlan "switch0" "1" "2 3 4 5 6t" >> $debug 2>&1
			if [ $iptv_enabled -eq 1 ]; then
				ucicfg_add_switch_vlan "switch0" "2" "1t 6t" >> $debug 2>&1
			fi
			if [ $voip_enabled -eq 1 ]; then
				ucicfg_add_switch_vlan "switch0" "3" "1t 6t" >> $debug 2>&1
			fi
			ucicfg_add_switch_vlan "switch0" "6" "1t 6t" >> $debug 2>&1
			;;
		"asus,rt-ac87u")
			ucicfg_add_switch "switch0" "1" "1" >> $debug 2>&1
			switch_ifname_lan="eth1.1"
			ucicfg_add_switch_vlan "switch0" "1" "1 2 3 5 7t" >> $debug 2>&1
			if [ $iptv_enabled -eq 1 ]; then
				switch_ifname_iptv="eth1.2"
				ucicfg_add_switch_vlan "switch0" "2" "0t 7t" >> $debug 2>&1
			fi
			if [ $voip_enabled -eq 1 ]; then
				switch_ifname_voip="eth1.3"
				ucicfg_add_switch_vlan "switch0" "3" "0t 7t" >> $debug 2>&1
			fi
			switch_ifname_wan="eth1.6"
			ucicfg_add_switch_vlan "switch0" "6" "0t 7t" >> $debug 2>&1
			;;
		"mynet-n750")
			ucicfg_add_switch "switch0" "1" "1" >> $debug 2>&1
			ucicfg_add_switch_vlan "switch0" "1" "0t 1 2 3 4" >> $debug 2>&1
			if [ $iptv_enabled -eq 1 ]; then
				ucicfg_add_switch_vlan "switch0" "2" "0t 5t" >> $debug 2>&1
			fi
			if [ $voip_enabled -eq 1 ]; then
				ucicfg_add_switch_vlan "switch0" "3" "0t 5t" >> $debug 2>&1
			fi
			ucicfg_add_switch_vlan "switch0" "6" "0t 5t" >> $debug 2>&1
			wan_mac=$(mtd_get_mac_ascii devdata "wanmac")
			;;
		"rb-951ui-2hnd")
			ucicfg_add_switch "switch0" "1" "1" >> $debug 2>&1
			ucicfg_add_switch_vlan "switch0" "1" "0 1 2 3 4" >> $debug 2>&1
			switch_ifname_lan="eth1"
			;;
		"tl-wdr4300")
			ucicfg_add_switch "switch0" "1" "1" >> $debug 2>&1
			ucicfg_add_switch_vlan "switch0" "1" "0t 2 3 4 5" >> $debug 2>&1
			if [ $iptv_enabled -eq 1 ]; then
				ucicfg_add_switch_vlan "switch0" "2" "0t 1t" >> $debug 2>&1
			fi
			if [ $voip_enabled -eq 1 ]; then
				ucicfg_add_switch_vlan "switch0" "3" "0t 1t" >> $debug 2>&1
			fi
			ucicfg_add_switch_vlan "switch0" "6" "0t 1t" >> $debug 2>&1
			;;
		"tl-wr1043nd")
			ucicfg_add_switch "switch0" "1" "1" >> $debug 2>&1
			ucicfg_add_switch_vlan "switch0" "1" "1 2 3 4 5t" >> $debug 2>&1
			if [ $iptv_enabled -eq 1 ]; then
				ucicfg_add_switch_vlan "switch0" "2" "0t 5t" >> $debug 2>&1
			fi
			if [ $voip_enabled -eq 1 ]; then
		...
		"tl-wr1043nd-v2")
			ucicfg_add_switch "switch0" "1" "1" >> $debug 2>&1
			ucicfg_add_switch_vlan "switch0" "1" "1 2 3 4 6t" >> $debug 2>&1
			if [ $iptv_enabled -eq 1 ]; then
				ucicfg_add_switch_vlan "switch0" "2" "5t 6t" >> $debug 2>&1
			fi
			if [ $voip_enabled -eq 1 ]; then
				ucicfg_add_switch_vlan "switch0" "3" "5t 6t" >> $debug 2>&1
			fi
			ucicfg_add_switch_vlan "switch0" "6" "5t 6t" >> $debug 2>&1
			;;
		"tl-wr841n-v1" |\
		"tl-wr941nd")
		...
		"wrt160nl")
			ucicfg_add_switch "switch0" "1" "1" >> $debug 2>&1
			ucicfg_add_switch_vlan "switch0" "1" "0 1 2 3 4t" >> $debug 2>&1
			switch_ifname_iptv="eth1.2"
			switch_ifname_voip="eth1.3"
			switch_ifname_wan="eth1.6"
			;;
		*)
			ucicfg_add_switch "switch0" "1" "1" >> $debug 2>&1
			ucicfg_add_switch_vlan "switch0" "1" "$switch_port_lan ${switch_port_cpu}t" >> $debug 2>&1
			if [ $switch_wan_eth1 -eq 0 ]; then
				if [ $iptv_enabled -eq 1 ]; then
					ucicfg_add_switch_vlan "switch0" "2" "${switch_port_wan}t ${switch_port_cpu}t" >> $debug 2>&1
				fi
				if [ $voip_enabled -eq 1 ]; then
					ucicfg_add_switch_vlan "switch0" "3" "${switch_port_wan}t ${switch_port_cpu}t" >> $debug 2>&1
				fi
				ucicfg_add_switch_vlan "switch0" "6" "${switch_port_wan}t ${switch_port_cpu}t" >> $debug 2>&1
			fi
			;;
	esac
...
set network.wan6.ifname="${switch_ifname_wan}"
